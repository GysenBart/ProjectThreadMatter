...
class Paddle(pygame.sprite.Sprite):
    ...
    def left(self):
        self.rect.x -= self.velocity            #self.rect.x is the position of the paddle, velocity is the step value
        if self.rect.left <= 0:
            self.rect.left = 0
    ...
    #This function makes it possible that the controller can set the position of the paddle
    #Used only in version 1.1!
    ...
    def update(self):
        ...
        #Added this lines so the sensor can control the paddle from left to right
        #This lines are only used in version 1.0!!! can be removed
        value = self.parent.getSensorValue()
        if value < 102:
            self.left()
        elif value > 152:
            self.right() 
